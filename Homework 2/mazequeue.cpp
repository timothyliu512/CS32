...
bool pathExists(char maze[][10], int sr, int sc, int er, int ec){
    ...
    coordQueue.push( Coord(sr,sc) );        // Push starting coord to stack
    maze[sr][sc] = '*';                     // Mark starting cell as visited
    
    while ( !coordQueue.empty() )           // As long as there's a possible coordinate
    {
        Coord end = Coord(er,ec);           // Ending coordinates
        Coord current = coordQueue.front();   // Get current coord
        coordQueue.pop();                   // Make room for new coord
        ...
        {
            coordQueue.push(Coord(r+1,c));  // New coord will be current coord
        ...
        {
            coordQueue.push(Coord(r,c+1));  // New coord will be current coord
