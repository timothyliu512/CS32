...
void Rabbit::move()
{
    // If rabbit poisoned, record its position before moving
    if (m_health < INITIAL_RABBIT_HEALTH)
    {
        m_arena->history().record(m_row,m_col);
    }
    
    // If rabbit is poisoned, won't move this turn but moves next turn
    ...
    // If health decreases, rabbit considered poisoned and now given move every other turn effect
    if (m_health < INITIAL_RABBIT_HEALTH)
    {
