...
//  Homework 4
//
//  Created by Timothy Liu on 5/11/23.
//
...
template<typename seqTemp>
int subsequence(const seqTemp& seq1, const seqTemp& seq2);

template<typename seqTemp>
void concatReverse(const seqTemp& seq1, const seqTemp& seq2, seqTemp& result);
...
int subsequence(const SequenceT& seq1, const SequenceT& seq2)
{
    ...
    for ( int i = 0; i < seq1.size() ; i++ ) // Go through seq1
    {
        ...
        for ( ; j < seq2.size() ; j++ ) // Go through seq2
        {
            ...
            typename SequenceT::valueType seq1Compare, seq2Compare;  // Get temp variable to compare seq1 and seq2 values
...
template<typename SequenceT>
void concatReverse(const SequenceT& seq1, const SequenceT& seq2, SequenceT& result)
{
    ...
    {
        typename SequenceT::valueType temp;                      // Temporarily store value to be copied over
